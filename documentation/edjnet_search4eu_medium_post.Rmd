...
The repo containing the code for the search4eu project is available under CC licence. We are also assembling a user guide as a wiki for the repo for those who would like to use Aleph for their own crawling and indexation projects.
...
Mathieu and Sylvain had already scrpaed documents from their local government : admnistrative registers of the Marseille city council for the first, local assembly reports of Seine-Saint-Denis cities for the second.

The operation is quite simple : you go into the command line and you point Aleph at what you want it to *"digest"*. In this case, a folder containing all the PDF. Not complicated but not very efficient either : without the extra data explaining what was in the file, the only intel Aleph could retrieve was what if could find inside the docs ... and it was pretty messy.

In practice : the Marseille registers were organized in two column format. Letting the Aleph program wonder : where does one decision starts and when does it end ? Where is that date ? Where's the title ? In the end, our batch were digested alright, but the nutrient for investigation was missing badly ...

## The numbers
Mathieu managed to pipe 281 Marseille administratives registers into Aleph in less than 10 minutes.
Sylvain made it digest couple of batches of municipal meeting report. For example : the 28 sessions of the city of Aubervilliers were maped in a few minutes.

## Lessons learned

**Aleph is not a file cabinet : it's a search engine. And without meta data, a search engine is as messy as the photo file of your smartphone** (when you haven't sorted it in two or three years).

We thought at first that the information contained in the docs would be sufficient for Aleph to self organize. But it was foolish. Not becaue Aleph is bad at it but because the docs come into so many size and shapes, you can't predict the metadata that will eventually be extracted from it !

We needed metadata. And, to gather some, we needed to go full Aleph stack and dig in Memorious.
...
We first asked our colleague from Civio where to look for. She provided us with a long list of websites, mainly national and local authorities : Boletin Oficial del Estado (the Spanish Official journal), his equivalent each autonomous provinces and cities, the Spanish Congress website, etc.
...
But this part was the easy one. Once we got to the page itself, we needed to target the right link to follow to get to the docs. And that's where things go complicated : some of those sites generate their pages as their are asked for (with dynamic pages). We hit a wall several times, trying to point at something that we *"saw"* but that wasn't available at a given url. We couldn't point at a place, we had to trace the path to it from a starting page. Memorious can do that, but you have to become a first class GPS device to get him where you want.

## The numbers
The main crawler we set up, targeting the Boletin Official del Estado, was able to store 79907 documents in couple of hours of running. We stoped it to save processing time and storage.
...
We now had the docs but also the right company names, dates and sectors ... those things we wish the original search engine had.

## The numbers
The original crawler for "antitrust and cartels" was able to retrieve 2886 documents in 2 hours.
Our second crawler, only following links from one case page, ended up storing 433 documents in 1 hour.

## Lessons learned

The first thing we realised here is that Aleph can be very user friendly. During our previous attempts, we sweat at building our crawlers and then took hours to analyze the docs they brought us back ... Now we knew out to connect aleph and memorious, we could see the result popping up in real time in the graphical interface !
...
* backend dev to setup the tools and manage the databases ;
* frontend dev to improve the access and build new entry points for users into the database ;
* natural language processing experts to improve the detection of entities (people, companies, places, etc) ;
...
1. documentation : the user guide, some translation to help our folks at OCCRP ;
2. contribution : Mathieu keeps pushing new improvement to the original repo and we're digging into the feature we think would be useful to add ;
3. Memorious :
  * the competition crawler needs to be extended, benefiting from the last updates we made ;
...
4. Aleph :
  * the competition Aleph would benefit from some improvement in the user experience (PDF rendering, research, etc.) ;
